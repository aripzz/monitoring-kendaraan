...
@section('title', 'Edit Kendaraan - Monitoring Kendaraan')
        ...
        <form action="{{ route('vehicles.update', $vehicle->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type"
                    ...
                    <option value="pengangkut_orang"
                        {{ old('type', $vehicle->type) == 'pengangkut_orang' ? 'selected' : '' }}>Pengangkut Orang</option>
                    <option value="pengangkut_barang"
                        {{ old('type', $vehicle->type) == 'pengangkut_barang' ? 'selected' : '' }}>Pengangkut Barang
                    </option>
                </select>
                @error('type')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="owner" id="owner"
                    ...
                    <option value="inhouse" {{ old('owner', $vehicle->owner) == 'inhouse' ? 'selected' : '' }}>Inhouse
                    </option>
                    <option value="rental" {{ old('owner', $vehicle->owner) == 'rental' ? 'selected' : '' }}>Rental</option>
                </select>
                @error('owner')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="date" name="next_service_date" id="next_service_date"
                    ...
                    value="{{ old('next_service_date', $vehicle->next_service_date ? date('Y-m-d', strtotime($vehicle->next_service_date)) : '') }}">
